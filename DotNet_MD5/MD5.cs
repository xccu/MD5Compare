...
namespace DotNetMD5
{
    ...
    public class MD5
    {
        ...
        private static long[] ConvertToWordArray(string sMessage)
        {
            ...
            lByteCount = 0;
                ...
                while (lByteCount < lMessageLength)
                {
                    ...
                    lWordArray[lWordCount] = lWordArray[lWordCount] | LShift(Convert.ToInt32(sMessage.Substring(lByteCount, 1).ToCharArray()[0]), lBytePosition);//use ToInt32 to avoid overflow
        ...
        private static string WordToHex(long lValue)
        {
            ...
            {
                var t = RShift(lValue, lCount * BITS_TO_A_BYTE);
                lByte = RShift(lValue, lCount * BITS_TO_A_BYTE) & m_lOnBits[BITS_TO_A_BYTE - 1];

                if (lByte > 0)//this judge for test to avoid error
                    WordToHex = WordToHex + (("0" + ToHex(lByte)).Substring(("0" + ToHex(lByte)).Length - 2));
                else
                    WordToHex = WordToHex + (("0" + ToHex(lByte)).Substring(("0" + ToHex(lByte)).Length));
            }
            return WordToHex;
        }
